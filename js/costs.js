...
export function costReport(state,finance,from,to){
 ...
 for(const e of [...entries.filter(e=>e.kind==='overhead'),...purchaseShares(finance).filter(p=>kinds.has(p.job_id)).map(p=>({valid_from:p.order_date,amount:p.amount}))])for(const mode of ['actual','planned']){const key=mode+e.valid_from.slice(0,7),pool=pools.get(key)||{cost:0,hours:0,missing:false};pool.cost+=Number(e.amount);pools.set(key,pool)}
 ...
 for(const j of result.values()){
  j.sharedPurchases=purchaseShares(finance).filter(p=>p.job_id===j.id&&p.shared&&p.order_date>=from&&p.order_date<=to).length;
  j.purchaseCost=purchaseShares(finance).filter(p=>p.job_id===j.id&&p.order_date>=from&&p.order_date<=to).reduce((n,p)=>n+Number(p.amount),0);
